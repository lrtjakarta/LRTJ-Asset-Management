...
@push('scripts')
    <script>
        (function() {
            const table = $('#tableMasterUOM').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('master.uom.data') }}",
                order: [
                    [4, 'desc']
                ],
                "dom": "<'row mb-2'" +
            ...
            $(document).on('click', '#btn-export-excel', function() {
                window.location = "{{ route('export.master_uom') }}";
            });
            ...
            $('#formMasterUOM').on('submit', function(e) {
                e.preventDefault();
                const $f = $(this);
                const payload = $f.serialize();

                $.post("{{ route('master.uom.save') }}", payload)
                    .done(function(res) {
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: "{{ route('master.uom.delete', ':uuid') }}".replace(':uuid',
                            uuid),
                        type: 'DELETE',
                        data: {
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            {{-- <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="#" class="btn btn-sm fw-bold btn-danger" data-bs-toggle="modal"
                    data-bs-target="#kt_modal_new_target">Add UOM</a>
            </div> --}}
                        ...
                        <div class="card mb-5 mb-xl-8">
                            ...
                            <div class="card-header border-0 pt-5">
                                <h3 class="card-title align-items-start flex-column">
                                    <span class="card-label fw-bold fs-3 mb-1">Master UOM Data</span>
                                </h3>

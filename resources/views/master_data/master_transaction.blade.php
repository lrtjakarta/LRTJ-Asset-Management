...
@push('scripts')
    <script>
        (function() {
            const table = $('#tableMasterTransaction').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('master.transaction.data') }}",
                order: [
                    [4, 'desc']
                ],
                "dom": "<'row mb-2'" +
            ...
            $(document).on('click', '#btn-export-excel', function() {
                window.location = "{{ route('export.master_transaction') }}";
            });
            ...
            $('#formMasterTransaction').on('submit', function(e) {
                e.preventDefault();
                const $f = $(this);
                const payload = $f.serialize();

                $.post("{{ route('master.transaction.save') }}", payload)
                    .done(function(res) {
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: "{{ route('master.transaction.delete', ':uuid') }}".replace(
                            ':uuid',
                            uuid),
                        type: 'DELETE',
                        data: {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted',
                            text: res.message || 'Transaction deleted.'
                        });
                    }).fail(function(xhr) {
                        Swal.fire({
                            icon: 'error',
                    ...
                    <div class="table-responsive">
                        <div class="card mb-5 mb-xl-8">
                            ...
                            <div class="card-body py-3">
                                ...
                                <table id="tableMasterTransaction"

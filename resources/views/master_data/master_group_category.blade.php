...
@push('scripts')
    <script>
        (function() {
            // DataTable
            const table = $('#tableMasterGroupCategory').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('master.group_category.data') }}",
                order: [
                    [4, 'desc']
                ],
                "dom": "<'row mb-2'" +
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $.get("{{ route('master.group_category.show', ':uuid') }}".replace(':uuid', uuid))
                    .done(function(res) {
                        ...
                        const d = res.data;
                        const $f = $('#formMasterGroupCategory');
            ...
            $('#formMasterGroupCategory').on('submit', function(e) {
                e.preventDefault();
                const $f = $(this);
                const payload = $f.serialize(); // includes uuid (if any), name, status

                $.post("{{ route('master.group_category.save') }}", payload)
                    .done(function(res) {
                        ...
                        $('#formMasterGroupCategory')[0].reset(); 
                    })
                    .fail(function(xhr) {
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: "{{ route('master.group_category.delete', ':uuid') }}".replace(
                            ':uuid',
                            uuid),
                        type: 'DELETE',
                        data: {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted',
                            text: res.message || 'Group Category deleted.'
                        });
                    }).fail(function(xhr) {
                        Swal.fire({
                            icon: 'error',
                        ...
                        <div class="card mb-5 mb-xl-8">
                            ...
                            <div class="card-header border-0 pt-5">
                                <h3 class="card-title align-items-start flex-column">
                                    <span class="card-label fw-bold fs-3 mb-1">Master Group Category Data</span>
                                </h3>
                            ...
                            <div class="card-body py-3">
                                ...
                                <table id="tableMasterGroupCategory"
    ...
    <div class="modal fade" tabindex="-1" id="kt_modal_add">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="formMasterGroupCategory">
                    @csrf

                    ...
                    <div class="modal-body py-4">
                        <div class="row g-4">
                            <div class="col-12">
                                ...
                                <div class="btn-group" role="group" aria-label="Transfer type" style="border: solid .5px black; width:100%;">
                                    <input type="radio" class="btn-check" name="tr-type" id="tr-type-tf-val" value="tf-val" checked>
                                    <label class="btn btn-outline-danger btn-sm" for="tr-type-tf-val">
                                        1. Partials/Full (Gross only)
                                    </label>

                                    <input type="radio" class="btn-check" name="tr-type" id="tr-type-carry" value="carry_over_gross_accum">
                                    <label class="btn btn-outline-danger btn-sm" for="tr-type-carry">
                                        2. Carry-Over (Gross + Accum)
                                    </label>
                                </div>
                            </div>
...
(function() {
    ...
    const money2 = v => new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(v ?? 0));
    const fmt2   = v => new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(v||0));

    const SHOW_URL_TPL = @json(route('assets.detail', '__UUID__'));
    const setBusy = (busy) => { if (busy) $btn.attr('data-kt-indicator', 'on').prop('disabled', true); else $btn.removeAttr('data-kt-indicator').prop('disabled', false); };
            ...
            ajax: {
                ...
                processResults: data => ({
                    results: (data?.data || data || []).map(it => ({
                        ...
                        text: (it.asset_code ? it.asset_code : '') + (it.description ? ' - ' + it.description : '')
                    }))
                })
            }
        });
        ...
        columns: [
            {
                ...
                render: function(data, type, row) {
                    ...
                    const url = SHOW_URL_TPL.replace('__UUID__', encodeURIComponent(row.asset_uuid));
                    return `<a href="${url}" class="text-primary fw-semibold">${data ?? ''}</a>`;
                }
            },
            { data: 'asset_name', name: 'asset_name' },
            {
                data: 'cap_date', name: 'cap_date', defaultContent: '',
                render: function(iso, type) {
                    if (!iso) return '';
                    if (type === 'sort' || type === 'type') return iso;
                    const d = new Date(iso);
                    return new Intl.DateTimeFormat('en-GB', { timeZone: 'Asia/Jakarta', day: '2-digit', month: 'long', year: 'numeric' }).format(d);
                }
            },
            {
            ...
            { data: 'opening_balance', render: money, className: 'text-end' },
            { data: 'total_value', render: money2, className: 'text-end' },
            { data: 'useful_life_months', className: 'text-end' },
            { data: 'ending_balance_prev_year', render: money2, className: 'text-end' },
            { data: 'remaining_useful_life_months', className: 'text-end' },
            { data: 'transfers_in', render: money, className: 'text-end' },
            { data: 'transfers_out', render: money, className: 'text-end' },
            { data: 'adjustment_depreciation', render: money, className: 'text-end' },
            { data: 'depr_expense', render: money, className: 'text-end' },
            {
            ...
            { data: 'ending_balance', render: money, className: 'text-end' },
        ]
    });
    ...
    async function refreshTransferCap() {
        ...
        if (!fromUUID || !date) { $('#tr-cap-help').text(''); return; }
    ...
    async function refreshCarryPreview() {
        ...
        try {
            const res = await $.get("{{ route('depreciation.mv.carryover.preview') }}", {
                from_asset_uuid: fromUUID, actual_date: date, amount: amount
            });
            const d = res || {};
    ...
    const setBusyTransfer = (b) => { if (b) $btnSave.attr('data-kt-indicator', 'on').prop('disabled', true); else $btnSave.removeAttr('data-kt-indicator').prop('disabled', false); };
    ...
    $('#form-transfer').on('submit', async function(e) {
        ...
        if (!fromUUID || !toUUID) { toastr?.error('Please pick both assets'); return; }
        if (fromUUID === toUUID)  { toastr?.error('From/To asset cannot be the same'); return; }
        if (!(amount > 0))        { toastr?.error('Amount must be greater than 0'); return; }
        if (!date)                { toastr?.error('Actual Date is required'); return; }

        if (type === 'tf-val') {
            const capRem = Number($('#tr-amount').data('capRemaining') || 0);
            if (capRem && amount > capRem) { toastr?.error('Amount exceeds max allowed for that month.'); return; }
        } else {
            const capGross = Number($('#tr-amount').data('capGross') || 0);
            if (capGross && amount > capGross) { toastr?.error('Amount exceeds max carry-over (gross remaining).'); return; }
        }

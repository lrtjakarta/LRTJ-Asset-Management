...
class AssetsController extends Controller
{
    ...
    public function datatable(Request $request)
    {
        ...
        return DataTables::of($q)->make(true); // or ->toJson()
    }
    ...
    public function update(
        ...
        $newGroup = $groupBuilder->handle(
            ...
            $v->kode_asset_type,
            $v->kode_category,
            $v->kode_category_2 ?? null,
            $v->sub_category
        );

        $recode = $newGroup !== $asset->kode_group_category;

        DB::transaction(function () use ($v, $asset, $recode, $newGroup, $num) {
            ...
            if ($recode) {
                ...
                $asset->qrs()->update(['is_active' => false]);
                AssetsQr::create([
                    'asset_uuid'   => $asset->uuid,
                    'qr_data'      => $asset->asset_code,
                    'is_active'    => true,
                    'generated_at' => now(),
                ]);
            }
        ...
        return response()->json([
            'ok' => true,
            'message' => $recode ? 'Asset recoded' : 'Asset updated',
            'asset_code' => $asset->asset_code,
            'kode_group_category' => $asset->kode_group_category,
        ]);
    }
    ...
    {
        $asset->delete(); // model hook will soft-delete children

...
class SatoRfidPrinter
{
     ...
     * $size: '100x40' (default) atau '60x25'
     */
    public function printByAssetUuids(array $assetUuids, string $size = '100x40'): void
    {
        ...
        [$wDots, $hDots] = match ($size) {
            '60x25' => [$this->mmToDots(60), $this->mmToDots(25)],
    ...
    protected function buildInitBlock(int $wDots, int $hDots): string
    {
        ...
        $maxH = max(0, $wDots - 1);
        $maxV = max(0, $hDots); // ikut gaya template awal kamu
    ...
    protected function buildPrintBlock(Assets $asset, AssetsRfid $rfid, int $wDots, int $hDots, string $size): string
    {
        ...
        if ($size !== '60x25') {
            ...
            // =========================
            // TEMPLATE 60 x 25 (NEW)
            // - tinggi cuma ~200 dots @203dpi, jadi harus compact
            // - QR diperkecil & kanan
            // - text dipadatkan (hindari overflow)
            // =========================

            // Header kecil (lebih kecil dari 60x40)
            $cmd .= $esc . "%0"
                .  $esc . "H0016"
                .  $esc . "V00020"
                .  $esc . "P02"
                .  $esc . "RH0,SATO0.ttf,0,026,026," . $header;

            // QR kanan (lebih kecil)
            // module 3 => lebih kecil dari sebelumnya
            $qrModule = 3;
            // estimasi area QR ~130 dots
            $qrEstimateSize = 130;
            $qrH = max(16, $wDots - $qrEstimateSize);
            $qrV = 10;
            ...
            // Text kiri (compact)
            // NOTE: karena tinggi mepet, kita ambil 4 baris penting:
            // asset_code, desc, owner, lokasi
            $cmd .= $makeTtfText('0016', '00058', $assetCode, '020', '022');
            $cmd .= $makeTtfText('0016', '00082', $desc,      '020', '022');
            $cmd .= $makeTtfText('0016', '00106', 'Owner: ' . $ownerCode,   '020', '022');
            $cmd .= $makeTtfText('0016', '00130', 'Lok: ' . $locationCode,  '020', '022');

            // Kalau kamu masih mau RFID tampil, pakai short (tapi ini risk overflow kalau owner/location panjang)
            $shortEpc = substr($epc, 0, 6) . '...' . substr($epc, -6);
            $cmd .= $makeTtfText('0016', '00154', 'RFID: ' . $shortEpc, '020', '022');
        }

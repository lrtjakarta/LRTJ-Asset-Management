...
class TransferApi extends Controller
{
    public function index(Request $request)
    {
        ...
        // Keep Eloquent (to preserve relationships) but join assets + master labels.
        $q = Transfer::query()
            ->select([
                'assets_transfers.*',
            ...
            // optional filters
            ->when(!empty($v['asset_uuid']), fn($x) => $x->where('assets_transfers.asset_uuid', $v['asset_uuid']))
            ...
            // free text across transfers + asset
            ->when(!empty($v['q']), function ($x) use ($v) {
                $like = '%' . trim($v['q']) . '%';
                $x->where(function ($w) use ($like) {
                    $w->where('assets_transfers.kode_status', 'ILIKE', $like)
        ...
        // Safe sorting (map friendly names to SQL expressions)
        $sortColumns = [
        ...
        if (!$paginationRequested) {
            $rows = $q->get();
            $data = $this->transformWithBatchedLabelsAndAsset($rows, $tz);
        ...
        $data      = $this->transformWithBatchedLabelsAndAsset($paginator->getCollection(), $tz);
    ...
    protected function transformWithBatchedLabelsAndAsset($rows, string $tz)
    {
        // --- Collect codes to batch-lookup (same as before) ---
        ...
        return $rows->map(function (Transfer $t) use ($mapUser, $mapStatus, $mapLoc, $tz) {
            ...
            // Asset fields pulled from the join (selected with aliases)
            $asset = [
